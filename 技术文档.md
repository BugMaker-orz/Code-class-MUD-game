...
> 阅读对象：**未接触过本项目的任何人**——评审老师、评审同学、新接手开发者
...
>
> **目标名识别（v2.5/M-02）**：`attack` 后的目标名同时接受中文名（`史莱姆`）、英文名（`slime`，大小写不敏感）、怪物短名；攻击向导接受 `向导`、`npc`、`guide`、`wizard`、`sage`。英文方向 `east/west/south/north`（含缩写 e/w/s/n）与中文方向等价。
...
```
玩家输入「攻击 哥布林」（或 attack goblin，英文目标名自动映射）
  → tryAttack 找到房间内活怪（空参数=第一只；中文名/英文名/怪物短名均可）
  → CombatSystem::playerAttackMonster(p, m, ctx)
       ...
       │           └─ 玩家 HP ≤ 0 → state = Dead
       └─ 命中且击杀：
            ① CombatSystem::handleMonsterDeath：发基础经验、基础金币、从怪物列表移除
               （M-03 修复后不再生成掉落物，避免一次击杀两轮掉落判定）
            ② tryAttack 调用一次 DropSystem::generateDrops → 自动拾取：
               金币入账；物品入背包，背包满→真实落在怪物死亡位置
            「你攻击哥布林，造成 6 点伤害！哥布林 被击败了！获得 6 经验，2 金币。」
```

> **v2.0 特性**：所有战斗消息都带怪物剩余血量 `（名字 HP 当前/满血）`，与攻击向导显示格式一致，玩家无需查看即可判断是否补刀。
>
> **v2.5（M-03）修复**：早期版本 `handleMonsterDeath` 和 `tryAttack` 各自调一次 `generateDrops`，一次击杀会进行两轮独立掉落判定，金币和物品都可能双倍产出。修复后 `handleMonsterDeath` 只负责经验/金币/移除，掉落判定全集中到 `tryAttack` 一处，一次击杀只掷一次掉落表。
...
### 14.2 掉落→背包的完整链路（v2.4 关键行为；v2.5 修正调用点）

```
tryAttack 击杀后调用一次 DropSystem::generateDrops（带死亡位置坐标）
  → TextGame 逐件处理：
...
**只掷一次**（v2.5/M-03）：掉落判定只在 `tryAttack` 里调一次；`CombatSystem::handleMonsterDeath` 只发基础经验与金币、不生成掉落物，避免一次击杀两轮独立掉落表。
...
③ （v2.5/M-04 新增）必填字段校验：level/p_level/p_exp/p_maxhp/p_hp/
   p_baseAtk/p_baseDef/p_gold/room 任一缺失 → 拒绝读档，提示"存档数据不完整"，
   不再用默认值静默重建（防止半截/损坏存档被当成正常进度）
④ cleanup() 清空当前局
...
| 17 | 英文方向（M-01） | 固定种子 42，`go east` 从起点房成功走到东侧战斗房，不再报"方向无效" |
| 18 | 英文目标名（M-02） | `attack slime` 正确命中史莱姆（HP 10→2→0）；`attack guide` 进入攻击向导流程 |
| 19 | 单次掉落（M-03） | 击杀史莱姆只结算一次：经验 +6、金币 +2，无第二轮掉落判定 |
| 20 | 坏存档拒绝读取（M-04） | 构造只含 `MUDTEXT_SAVE_V1`+`level=1` 的半截存档，输入"读取"被拒绝并提示"缺少字段 p_level" |

当前已通过 20 项端到端回归测试（见第 21 章），覆盖战斗、存档、结局、背包与命令解析全部关键路径。
...
"固定随机种子（seed 42）命令流回归 + 手工构造存档"两种方式：固定种子保证地图可复现，命令流逐帧核对输出；构造满背包存档（20 格）验证满包场景。当前 20 项端到端测试全部通过（见第 21 章）。
...
| # | 提交 | 说明 |
| --- | --- | --- |
| 16 | `11f5e74` | docs：加入实验报告 v3 与课堂展示 PPT |

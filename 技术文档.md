...
本项目为课程设计双版本之一（图形版 + 文字版）。文字版**不使用 WASD/二维地图渲染**，只保留 `src/text/` 专属层 + 共享核心层（角色/战斗/物品/地图），二维渲染代码不参与编译。
...
| 语言标准 | C++17 |
| 构建 | CMake ≥ 3.15 |
| 平台 | Windows（MSVC）/ Linux / macOS 均可编译 |
| 中文编码 | 源码 UTF-8；MSVC 由 CMake 强制 `/utf-8` 编译 |
| 控制台 | Windows 下 `setupConsole()` 将代码页切到 UTF-8 并开启 `ENABLE_VIRTUAL_TERMINAL_PROCESSING`（ANSI 彩色生效） |
...
└── src/
    ...
    ├── map/
    │   ├── Map.h/.cpp          #   房间地图数据
    │   └── DungeonGenerator.h/.cpp # 元气骑士式分层地牢生成器
    └── item/
